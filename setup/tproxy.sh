...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] 开始加载 IPv4 TProxy + ScuSSLVPN 规则 (链: $CHAIN_NAME)..." | tee -a "$LOG_FILE"

# 自动检测主网卡和IP（注意：这个检测可能在复杂网络环境下不准确，如果ScuSSLVPN的MASQUERADE行出错，请手动指定ens5）
...
# 注意：移除了 10.0.0.0/8，以确保 ScuSSLVPN 的 10.21.13.0/24 流量能被捕获进行 TPROXY
...
# 5. ScuSSLVPN VPN 兼容规则（允许转发和源地址伪装）
# 确保 IP 转发已开启
sysctl -w net.ipv4.ip_forward=1
# 允许 ScuSSLVPN VPN 网段的流量转发
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] ✅ IPv4 TProxy + ScuSSLVPN 规则加载完成" | tee -a "$LOG_FILE"
